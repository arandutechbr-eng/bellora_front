  ...
  return (
    <section className="container-page py-10">
      <div className="card overflow-hidden">
        <img src={professional.cover} className="h-72 w-full object-cover" />

        <div className="p-6 lg:p-8">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="flex gap-5">
              <img src={professional.avatar} className="-mt-16 h-28 w-28 rounded-3xl object-cover ring-8 ring-white dark:ring-slate-900" />
              <div>
                <h1 className="text-3xl font-extrabold">{professional.name}</h1>
                <p className="font-semibold text-brand-600">{professional.category}</p>
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_360px]">
          ...
          <div className="card p-6">
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              {professional.services.map((service) => (
                <span key={service} className="rounded-full bg-brand-50 px-4 py-2 text-sm font-bold text-brand-700 dark:bg-brand-900/30 dark:text-brand-100">
                  {service}
                </span>
              ))}
            </div>
          ...
          <div className="card p-6">
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              {professional.gallery.map((image) => (
                <img key={image} src={image} className="h-56 rounded-3xl object-cover" />
              ))}
            </div>
          </div>
              ...
              {reviews.map((review) => (
                <div key={review.id} className="rounded-2xl bg-slate-50 p-4 dark:bg-slate-800">
                  <div className="flex items-center gap-3">
                    <img src={review.avatar} className="h-10 w-10 rounded-full object-cover" />
        ...
        <aside className="card h-max p-6">
          ...
          <div className="mt-5 rounded-2xl bg-emerald-50 p-4 text-sm font-semibold text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-200">
            {professional.availableToday ? 'Disponível hoje' : 'Agenda sob consulta'}
          </div>
        </aside>
      </div>
    </section>
